...
tilelive.protocols['offlinesource:'] = source;
tilelive.protocols['offlinembtiles:'] = source;
...
source.getDir = function(uri) {
    return (uri.protocol === 'offlinesource:') ?
        path.join('data','sources',uri.dirname) :
        uri.dirname;
};

source.getMbTilesFile = function(uri) {
    return path.join('data','sources',uri.dirname);
};
...
source.refresh = function(rawdata, callback) {
    ...
    // local mbtiles
    if (uri.protocol === 'offlinembtiles:') {
        if (cache[id]) return callback(null, cache[id]);
        new MBTiles(rawdata.file, function(err, mbtiles) {
            if (err) return callback(err);
...
source.save = function(rawdata, callback) {
    ...
    function done(err, loaded) {
        ...
        source.thumbSave(id, path.join(source.getDir(uri),'.thumb.png'));
        ...
        tm.writefiles(source.getDir(uri), files, function(err) {
            if (err) return callback(err);
...
source.info = function(id, callback) {
    ...
    case 'tmpsource:':
    case 'offlinesource:':
        var filepath = path.join(source.getDir(uri),'data.yml');
        ...
        break;
    case 'offlinembtiles:':
        var file = source.getMbTilesFile(uri)
...
source.thumbSave = function(id, dest, callback) {
    ...
    var uri = tm.parse(id);
    dest = dest || path.join(source.getDir(uri),'.thumb.png');
