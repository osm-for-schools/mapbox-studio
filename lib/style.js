...
tilelive.protocols['offlinestyle:'] = style;
...
style.getDir = function(uri) {
    return (uri.protocol === 'offlinestyle:') ?
        path.join('data','styles',uri.dirname) :
        uri.dirname;
};
...
style.save = function(rawdata, callback) {
    ...
    style.refresh(rawdata, function(err, loaded) {
        ...
        tm.writefiles(style.getDir(uri), files, function(err) {
            if (err) return callback(err);
...
style.info = function(id, callback) {
    ...
    if (uri.protocol !== 'tmstyle:' && uri.protocol !== 'tmpstyle:'
            && uri.protocol !== 'offlinestyle:')
        return callback(new Error('Invalid style protocol'));
    var project_path = path.join(style.getDir(uri),'project.yml');
    return fs.readFile(project_path, 'utf8', function(err, data) {
        ...
        function readbookmarks() {
            ...
            data._bookmarks = [];
            var bookmarks_path = path.join(style.getDir(uri),'bookmarks.yml');
        ...
        function readstyles() {
            ...
            fs.readFile(path.join(style.getDir(uri), basename), 'utf8', function(err, mss) {
                if (err && err.code !== 'ENOENT') return callback(err);
...
style.thumb = function(id, callback) {
    var uri = tm.parse(id);
    return fs.readFile(path.join(style.getDir(uri),'.thumb.png'), function(err, buffer) {
...
style.thumbSave = function(id, dest, callback) {
    ...
    var uri = tm.parse(id);
    dest = dest || path.join(style.getDir(uri),'.thumb.png');
...
style.toPackage = function(id, dest, callback) {
    ...
    if (dest.writable && dest.setHeader) {
        var basename = path.basename(style.getDir(uri), '.tm2');
